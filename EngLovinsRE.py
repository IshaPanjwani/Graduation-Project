...
def K(base):
    ...
    return re.match('[a-z]{3,}(l|i|(e.u))$$', base)
    # c = base[-1]
    # cc = base[-3]
...
def L(base):
    ...
    return re.match('[a-z]*([^ux].|([^s]o))$$', base)
    # c1, c2 = base[-2:]
...
def M(base):
    ...
    return re.match('[a-z]*[^acem]$$', base)
    # c = base[-1]
    # return c != "a" and c != "c" and c != "e" and c != "m"


def N(base):
    return re.match('(^[a-z]{2}[^s])|(^[a-z]{4,})$$', base)
...
def S(base):
    ...
    return re.match('[a-z]*((rd)|(^tt)t)$$', base)
    l2 = base[-2]
...
def T(base):
    ...
    return re.match('[a-z]*(s | (t[^o]))$', base)
    # c1, c2 = base[-2:]
...
def Y(base):
    ...
    return re.match('[a-z]*(in)$', base)
    # return base[-2:] == "in"


def Z(base):
    # Z  Do not remove ending after f
    return re.match('[a-z]*[^f]$', base)
    # return base[-1] != "f"


def a(base):
    # a  Remove ending only after d, f, ph, th, l, er, or, es or t
    return re.match('[a-z]*(d | f | ph | th | l)?(er | or | es | t)?$', base)
    # c = base[-1]
    # l2 = base[-2:]
...
def b(base):
    ...
    return re.match('[a-z]{2,}[^(met) | (ryst)]$', base)
        ...
        if word.endswith(target):
            if len(endingrule) > 2:
                if ((0 - (len(target) + 1)) + len(word)) < 0:
                    ...
                    if c in exceptafter:
                        return word
            return word[:+1 - len(target)] + newend
    return word
