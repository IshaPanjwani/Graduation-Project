class PorterStemmer:
    ...
    def step1b(self, word):
        ...
        if flag:
            if word.endswith('at') or word.endswith('bl') or word.endswith('iz'):
                word += 'e'
            elif self.doubleCons(word) and not self.endsWith(word, 'l') and not self.endsWith(word,
                                                                                              's') and not self.endsWith(
                    word, 'z'):
                word = word[:-1]
    ...
    def step4(self, word):
        if word.endswith('al'):
            ...
            word = self.replaceM1(word, 'ier', 'y')
        elif word.endswith('iest'):
            word = self.replaceM1(word, 'iest', 'y')
